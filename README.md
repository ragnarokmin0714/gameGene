...
> **Scope & ethics.** MemGene is for single-player / offline games and your own
...
MemGene is a Cargo workspace of three crates, each with one job:

| Crate | Role | Contents | OS-dependent? |
|-------|------|----------|---------------|
| **`memgene-core`** | The brain (rules) | Value types, the scan engine (first scan → next scan), the cheat table, and the `MemorySource` trait. | No — pure logic, unit-tested on any platform. |
| **`memgene-platform`** | The hands (touching the OS) | Implementations of `MemorySource`: Windows (`ReadProcessMemory`/`WriteProcessMemory`), Linux (`/proc`), and a mock. The right one is chosen at compile time via `cfg`. | Yes — all `unsafe` syscalls live here. |
| **`memgene-app`** | The face (UI) | The egui desktop app (`memgene` binary) that wires core + platform together. | Through the two crates above. |

The key idea: **`memgene-core` never calls the operating system.** It talks to
...
`memgene-platform`; core doesn't change.

```
  memgene-app        ──  egui desktop UI; the binary you launch
       │  calls
       ▼
  memgene-core       ──  scan engine, cheat table, value types;
       ▲                 defines the MemorySource trait; no OS calls
       │  implements the trait
  memgene-platform   ──  Windows / Linux / mock; the real syscalls
```
...
cargo build --release        # release binary at target/release/memgene[.exe]
cargo test  --workspace      # run the engine + cheat-table + backend tests
```

MemGene targets **64-bit** processes.
...
1. `cargo build --release` → `target\release\memgene.exe`.
...
3. Launch your single-player game, then in MemGene: pick the process in the
   left list → **Attach**.
...
1. Switch to **Desktop Mode** (KDE Plasma) — MemGene is a windowed GUI.
2. Install Rust with rustup. SteamOS's system partition is read-only, but
   ...
   copy the binary.)
3. `cargo build --release`, run `./target/release/memgene`.
...
`mach_vm_write` in `memgene-platform` (and code signing / entitlements to attach
to other processes).
...
freeze. Tables save/load as `.mgtable` JSON files.
...
- **Single-player / offline only.** Editing values in a game you own, running on
  ...
  cheat code. That is MemGene's intended use.
